...
?>    <?php
    ...
    $subjectt = $_SESSION['subbro'];
    require "connection_server.php";
    
    
       $query = "Select * from quiz_answers where id='$studentid' and subject='$subjectt';";
    $result = mysqli_query($mysqli,$query);
     $queryq = "Select * from theory_answers where id='$studentid' and subject='$subjectt';";
    $resultq = mysqli_query($mysqli,$queryq);
     $queryw = "Select * from pdf_answers where id='$studentid' and subject='$subjectt';";
    $resultw = mysqli_query($mysqli,$queryw);
    ?>
<!doctype html>
<html lang="en">
<head>
...
<section style="display: block;">
    ...
    if
        ($result->num_rows !=0){
         ...
         {
        echo "<p> ASSIGNMENT-".$rows["assignment_no"]."-".$rows["score"]."</p>"; 
    }
        }
    ?>
    <h1>THEORY</h1>
    ...
    if
        ($resultq->num_rows !=0){
         ...
         {
        echo "<p> ASSIGNMENT-".$rowq["assignment_no"]."-".$rowq["score"]."</p>"; 
    }
        }
    ?>
    <h1>PDF</h1>
    ...
    if
        ($resultw->num_rows !=0){
         ...
         {
        echo "<p> ASSIGNMENT-".$roww["assignment_no"]."-".$roww["score"]."</p>"; 
    }
        }
    ?>
    
    <a href="11.php"><h2>BACK</h2></a>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ans2 = $_POST["answer2"];
$subject =$row["subject"];
$queryd = "Select * from faculty where subject='$subject';";
    $resultd = mysqli_query($mysqli,$queryd);
    $rowd = $resultd->fetch_assoc();
    $facultyid=$rowd["id"];

    $sql = "INSERT INTO theory_answers (id,type,subject,branch,section,test_no,assignment_no,ans_1,ans_2)"
            . "VALUES ('$studentid','theory','$subject','cse','b',$testno,1,'$ans1','$ans2')";
