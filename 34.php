...
$query = "Select * from theory_answers where id='$sid' and assignment_no='$testnum' and type='theory';";
$result = mysqli_query($mysqli, $query);
$row = $result->fetch_assoc();
$querys = "Select * from student where id='$sid';";
$results = mysqli_query($mysqli, $querys);
$rows = $results->fetch_assoc();
$queryz = "Select * from theory_questions where id='$facultyid' and assignment_no='$testnum' and type='theory' and subject='$sub';";
$resultz = mysqli_query($mysqli, $queryz);
...
<section style="display: block;">
	<div class="container">
		...
		<div class="row margin-top-20">
			  ...
			  <h3><?php echo $rows["name"] ?></h3> <h3><?php echo $rowz["assignment_no"] ?></h3> 
        <form action="34.php" method="post" >
            <h3> 1:<?php echo $rowz["question_1"] ?> </h3> 
            <p>ANS:- <?php echo $row["ans_1"] ?></p>  <p> 10 </p>  marks:-<input type="number" name="o1" required>

            <h3> 2.<?php echo $rowz["question_2"] ?> </h3>
            <p>ANS:- <?php echo $row["ans_2"] ?> </p>  <p> 10 </p>  marks:-<input type="number" name="o2" required>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $one = $_POST["o1"];
    $two = $_POST["o2"];
$numbu=$rowz["assignment_no"];
    $reply = ($one + $two) / 2;
    $qur = "UPDATE theory_answers SET score='$reply' WHERE assignment_no=$numbu";
    $result = $mysqli->query($qur);
    if ($result) {
        echo "<script type='text/javascript'>  window.alert('corrected successfully!')      
 document.location = '25.php'; </script>";
    } else {
        echo "<h1>could not be submitted</h1>";
    }
